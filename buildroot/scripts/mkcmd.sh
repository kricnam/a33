...
function select_qt()
{
    ...
    while true ; do
        ...
        if [ -z "${choice//[0-9]/}" ] ; then
            if [ $choice -ge 0 -a $choice -lt $cnt ] ; then
                export LICHEE_QT="${qts[$choice]}"
                echo "export LICHEE_QT=${qts[$choice]}" >> .buildconfig
                break
            fi
        fi
        printf "Invalid input ...\n"
...
function cp_qt4.8.7()
{
    rm -fr ${LICHEE_BR_OUT}/target/opt/qt-4.8.7
    mkdir -p ${LICHEE_BR_OUT}/target/opt/

    #cd ${LICHEE_BR_DIR}/target/sinlinx/; \
    (
    cd /opt; \
        if [ ! -d "./qt-4.8.7" ]; then \
            echo "can't find qt-4.8.7, please build qt first"; \
            echo "eg."; \
            echo "  cd /root/work/sinlinx/a33/qt-4.8.7/"; \
            echo "  ./build.sh"; \
            return 1; \
        fi 
    )

    cp -fr /opt/qt-4.8.7 ${LICHEE_BR_OUT}/target/opt/
}

function regen_rootfs()
{
    ...
    if [ ${LICHEE_QT} = "none" ] ; then
        rm -fr ${LICHEE_BR_OUT}/target/opt/qt-4.8.7
    elif [ ${LICHEE_QT} = "qt4.8.7" ] ; then
        cp_qt4.8.7
        [ $? -ne 0 ] && mk_error "can't found qt-4.8.7" && return 1
    fi
    cp -rf ${LICHEE_BR_DIR}/target/sinlinx/rootfs/* ${LICHEE_BR_OUT}/target/
    #rm -rf ${LICHEE_BR_OUT}/target/lib/modules/*
    #cp -rf ${LICHEE_KERN_DIR}/output/lib/modules/* ${LICHEE_BR_OUT}/target/lib/modules/
